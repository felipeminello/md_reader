...
- Non-SDK dependencies: `flutter_bloc` (state management), `file_picker` (native file dialog), `desktop_drop` (drag-and-drop file target on the empty-state screen), `flutter_markdown` (rendering), `flutter_mermaid` (pure-Dart Mermaid diagram rendering), `flutter_svg` (SVG images in documents), `markdown` (AST types used by the custom element builder), plus the original `cupertino_icons`. Add new packages via `flutter pub add <name>` so `pubspec.yaml` and `pubspec.lock` stay in sync.
